...
//!
//! Параметры потребляет машина споров при M2-порте; до него канистра — канон их ХРАНЕНИЯ.
...
pub struct DisputeParams {
    ...
    /// Потолок суммы задания (micro-USDC); 0 = не ограничен (формула D_max — калибровка M2).
    pub d_max_micro: u64,
}

impl Default for DisputeParams {
    fn default() -> Self {
        // Дефолты спеки §7 + текущие (FAST) окна machine.ts — сверяются при M2-порте.
        DisputeParams {
