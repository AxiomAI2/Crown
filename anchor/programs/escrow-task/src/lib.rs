...
//! ADR 0015; экономика/исходы — `docs/yellow-paper.md` §4.
//!
...
//!   * Спор решает `resolve_dispute` ограниченным резолвером (может только выбрать сторону
//!     donor|streamer). С M2 (ADR 0021) RESOLVER = тресхолд-адрес core-канистры ICP: сторону
//!     выбирает голосование в арбитре канистры, подпись — консенсус её узлов; bounded-свойство
//!     (не может украсть, только выбрать сторону) сохраняется как защита в глубину.
//!
//! ⚠️ НЕ СКОМПИЛИРОВАНО в окружении разработки (нет хост-gcc для proc-макросов; нет SOL на деплой —
//!    devnet-фасет 429). Сборка/тест/деплой — по `docs/runbook.md` (раздел «Эскроу-программа») на машине с тулчейном.
...
// резолвером себя и делает clawback после выполнения. Devnet-адреса; на мейннете — свои константы + редеплой.
...
pub mod escrow_task {
    ...
    /// Резолвер помечает эскроу СПОРНЫМ (в арбитре канистры открыт спор, M2) → `resolve_timeout` его
    /// больше не трогает, пока резолвер не закроет спор через `resolve_dispute`. Закрывает гонку
    /// «таймаут опередил голосование» (ADR 0017). Подписывает тресхолд-адрес канистры (ADR 0021).
    ...
    /// Резолв СПОРА (bounded): только зафиксированный `resolver` и только выбор стороны.
    /// Получатели/сумма зашиты в эскроу — украсть/перенаправить нельзя. С M2 (ADR 0021) сторону
    /// определяет голосование арбитра канистры, подписывает её тресхолд-адрес.
...
pub struct Escrow {
    ...
    pub resolver: Pubkey,  // bounded-резолвер спора; с M2 — тресхолд-адрес канистры (ADR 0021)
    pub amount: u64,
