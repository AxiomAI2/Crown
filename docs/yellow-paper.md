...
- Санкции спора — только репутация (денежных наказаний нет — решение владельца, §18.5-8b):
  инициатор выиграл → `DISPUTE_WON +10`; проиграл → `DISPUTE_LOST −50` (ложный спор
  EV-отрицателен). Свёртки журнала стали знаковыми с клампом на нуле.
...
| `/studio/settings` | секции: Адрес выплат (аттестация H1, только chain) / Описание / Тиры / Донаты (минимумы, лимит текста) / Имена и показ / Модераторы / Параметры споров (только icp: подпись владельца → канистра, таймлок §8.9; поле D_max из формы убрано — мёртвое, §18.5-8c); draft-модель с плавающим «Сохранить» |
...
1. Оверлей/SSE в коде ОТСУТСТВУЕТ (был в Фазе 2, удалён при ADR 0005). Возврат оверлея —
