...
оверлей `CanisterDisputeBlock` (открытое табло/вердикт/подпись резолвера), голоса — те же
кнопки `vote`, которые IcpDataProvider уводит в арбитр подписью кошелька), `TaskFeedRow`
(строка в общей ленте), `DisputeTally`, `DisputePage` (`/c/[handle]/dispute/[taskId]`,
серверная пагинация голосов по 50, фильтры/сортировки/поиск), `EscrowTaskRules` (окна рендерятся из WINDOWS — не расходятся с активным режимом).
...
### 11.1 Контракт (`provider.ts`) — 32 метода (+2 опциональных)
...
Опциональные (только IcpDataProvider, M1 ADR 0021): `getDisputeParams?, setDisputeParams?` —
...
### 11.5 IcpDataProvider [мост в КОСТЬ №2] — режим `icp` (M1, ADR 0021)

`extends ChainDataProvider`, переопределены читающие методы `getStanding`/`getLeaderboard`/
`getDonorOverview`: браузер читает канон репутации напрямую из HTTP-экспорта core-канистры
...
сервера; каналы без ончейн-активации (mock-эпоха) остаются с серверными цифрами. Тиры — `resolveTier` по конфигу канала (кожа);
имена доноров на лидерборде — join с серверным лидербордом (косметика; сервер упал → цифры
без имён). Внутренние вызовы записи (`createDonation`→standing) остаются на api-пути —
переключено только витринное чтение. Дельта канона на переходе — §18.5-8a. Включение:
`NEXT_PUBLIC_DATA_SOURCE=icp` + `NEXT_PUBLIC_ICP_CANISTER_URL`; откат — `=chain`
(миграция M1, `docs/migration-plan.md` §3). Серверный индексер в icp-режиме работает
...
> **Чистка 2026-07-02**: на устаревшие спеки повешены баннеры «⚠️ Статус» с отсылкой сюда;
> конкретные ложные утверждения исправлены. Сами спеки сохранены — это продуктовый замысел.

1. ✅ Оверлей/SSE: ложная галочка «[x] проверено» в ROADMAP снята с пояснением (оверлей был в
   Фазе 2, удалён при ADR 0005; в коде отсутствует); баннер в `screens.md`. Возврат оверлея —
   отдельный шаг стримерского тулинга.
2. ✅ CLAUDE.md §7 исправлен: очки дробные (micro-точность), не целые.
3. ✅ `core-spec.md` / `screens.md`: баннеры про фиксированную формулу (ADR 0007) и отсутствующий
   оверлей.
4. Версионирование конфига: баннер в `data-model.md`; сам механизм в коде по-прежнему отсутствует
   (§18.4-1).
5. ✅ Спека игры: баннер (окна принятия нет, дедлайны от создания, активен FAST_TEST_WINDOWS;
   §12-возврат при тейкдауне не реализован намеренно — WP-3/CR-2).
6. ✅ `crypto/spec.md`: баннер (стек web3.js v1 по ADR 0004, а не kit/gill; §6-якорь текста
   отдельной инструкцией не реализован — хэш уже едет в memo.m).
7. ✅ `architecture.md`: «карантин с retention» → честно «retention — план, TTL в коде нет».
...
8c. **Экономика споров упрощена (решения владельца 2026-07-05; спека v1.1 переопределена,
   баннер в ней стоит).** (1) Депозит/пошлина/залог удалены ЦЕЛИКОМ — «не сжигаем ничьи
   ...
   (текст про депозит заменён на честное «−50 репутации за проигрыш»).
...
- **КОСТЬ №2 (строится, ADR 0021)**: `canister/core/src/` — `donation.rs`/`reputation.rs`
  ...
  вес-снимки из журнала, √-кворум, учётный депозит, финализация таймером → вердикт + журнал;
  ...
  /standing, /leaderboard, /dispute-params GET/POST), `lib.rs` (эндпоинты/таймеры).
