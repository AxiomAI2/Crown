...
### 11.1 Контракт (`provider.ts`) — 32 метода
...
11. Хардкоды-дубли: ✅ `IS_CHAIN` (wallet-connect) и `SIWS_STORAGE_KEY` (dev/db ↔ chain-provider)
    ...
    textarea задания 280 vs `messageMaxLen`, `PRESETS` ×2, `HEADER_H`, TREASURY в escrow-smoke,
...
14. Мелкие UX-разрывы (кандидаты на отдельные фиксы, не мусор): блок донора не отражён в
    `canDonate`; `minReputationToTask` не префлайтится до `fund` (§18.3-5, деньги!); `tierChanged`
    при первом донате всегда false; REINSTATE с двумя целями не снимает полный бан кошелька;
    голос весом 0 занимает дедуп-слот.
