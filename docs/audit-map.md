...
| A2 | **MEDIUM** — wash-донат/self-dealing: нет проверки `donor ≠ payout/owner` → владелец с payout=свой кошелёк копит репутацию за ~3% комиссии (§4.3/§4.4) | закрыто | `createDonation`→`SELF_DONATION`; ончейн-ингест self → `pointsDelta=0`; эскроу→`SELF_TASK` |
