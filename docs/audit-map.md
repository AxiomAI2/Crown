...
Внешний аудит (нумерация A — коммитмент текста, репутация как продукт):

| Находка | Описание | Статус | Фикс |
|---------|----------|--------|------|
| A1 | **HIGH** — `hashContent` = FNV-1a 32 бита служил ончейн-коммитментом `memo.m` (сверка текста в `ingest`) и ключом кэш-вердикта → мгновенный второй прообраз: подмена текста чужого доната через публичный `ingestSignature(sig, evilText)` (поздняя привязка при `!existing.message`, §4.6) + коллизия с CLEAR (обход авто-модерации) | закрыто | `hashContent` → полный **SHA-256** (Web Crypto, async; браузер+сервер); callers awaited |
| A2 | wash-донат/self-dealing: `donor === payout/owner` копит репутацию за ~3% комиссии | **умышленное допущение (не баг)** | Репутация локальна и непередаваема (§4.3), плата ~3%+газ = proof-of-spend; блок «того же кошелька» бессмыслен — тривиально обходится вторым кошельком (Сибил, недоказуемо на цепочке). Ценность накрутки ограничена своим же каналом. Не патчим |
| A3 | **LOW→MED** — дробление доната удваивало очки: `pointsForAmount` round-half-up супераддитивен (`round(0.5)·2 > round(1)`, на любой .5) | закрыто | **дробные очки 1:1** (2.5 USDC → 2.5 очка): точное без округления НЕЙТРАЛЬНО к дроблению (0.5+0.5=1.0). Свёртка в целых micro-очках (детерминизм §4.4); `points_delta` bigint→numeric. Заменило промежуточный floor |

Сверка кода с whitepaper v1.1 (пространство имён **WP**, 2026-07-02). Ядро экономики сходится (матрица
исходов §6, три замка §7, репутация §8, некастодиальность). Нереализованные механики спеки — депозит спора,
D_max молодых каналов, commit-reveal ≥$50, рейт-лимиты, порог спора как рычаг стримера — это план (G2/G3b в
ROADMAP), не находки. Находки:

| WP | Severity | Находка | Статус | Решение |
|----|----------|---------|--------|---------|
| WP-1 | LOW | Payout-адрес канала не исключён из жюри спора (v1.1 §11 требует «стример и его выплатные адреса»); исключены донор и owner (`handlers.ts` vote) | **умышленное допущение** | Адресные блокировки обходятся вторым кошельком так же тривиально, как в A2 (сибил недоказуем на цепочке) — глобально ничего не меняет; реальный замок — вес=деньги + публичность кластера (§9 спеки). Не патчим |
| WP-2 | MED | Текст HELD/HIDDEN-задания уходил ЛЮБОМУ вызывающему из `gameQuery list/get/disputeVotes` — приватность держалась только на клиентском `canSeeText`, в отличие от донатов (серверный `redactDonation`, §4.6) | закрыто | `redactTask` в queries + `isChannelManager` в `GameContext`: приватный текст видят менеджеры канала и сам донор; операторский тейкдаун прячет ото всех (паритет с ядром) |
| WP-3 | — | Тейкдаун нелегальщины НЕ делает «спор воид + донат донору + депозит назад» (обещание v1.1 §12) — только снятие текста + инцидент + ADMIN_VOID/бан | **задокументированное ограничение** | Гарантированный возврат оператором требует admin-key над эскроу → запрещён (§14 спеки, §4.1). Деньги идут по правилам эскроу; спека игры §12 исправлена под реальность |
| WP-4 | MED | `create` задания обходил рычаги канала: `minDonation`/`minDonationWithText` и `messageMaxLen` не применялись (v1.1 §10: «задание дороже простого доната») | закрыто | `BELOW_MIN`/`TOO_LONG` в `handlers.create` (сервер — истина) + префлайт ДО фандинга в chain-провайдере (эскроу необратим — отказ после fund заморозил бы деньги) + валидация в панели |
| WP-5 | UX | Донор не предупреждён, что при игноре стримера деньги морозятся на ВЕСЬ срок сдачи: отдельного 72ч-окна принятия ончейн нет, pending истекает по донорскому сроку (коридор до 90 дн против «1ч…7дн + 72ч» v1.1) | закрыто (UX) | Панель: варнинг при сроке > 7 дней (заморозка до N дней, отмена только в грейс) + повтор в диалоге подтверждения |

Аудит цензуроустойчивости (пространство имён **CR**, 2026-07-02). Вопрос: где централизованная модерация
может двинуть ДЕНЬГИ или ИСХОД спора. Итог: **деньги цензуроустойчивы** (ончейн — permissionless
`resolve_timeout` покрывает все нетерминальные состояния вкл. Disputed→стример; нет застрявших/кражи/редиректа;
happy-path без ключа оператора; забаненный забирает ончейн напрямую — бан только app-слой). Но **ИСХОД спора**
операторуправляем через несколько рычагов (структурный корень: `tally` no_quorum/tie → to_streamer + подавление
явки жюри). Находки:

| CR | Severity | Находка | Статус | Решение |
|----|----------|---------|--------|---------|
| CR-1 | MED→HIGH | `ADMIN_VOID` (репутация=вес голоса) до снэпшота спора зануляет вес присяжного + голодит кворум → to_streamer. Рычаг неограничен (произвольная цель, нет привязки к нелегальщине, один ключ, лог самозаверенный) | закрыто | `computeVoteWeightAsOf` (reputation.ts) — вес/кворум считаются БЕЗ `ADMIN_VOID`; `reputationAsOf` в провайдере переведён на неё. Наказания сохранены: void бьёт по СТАТУСУ (`computePoints`: тир/перки), `DISPUTE_LOST` остаётся в весе, вырезать из голосования — полный бан кошелька |
| CR-2 | MED→HIGH | Тейкдаун текста в окне голосования ослепляет жюри → ниже кворума → to_streamer (`isContentBlocked`→`redactTask`, перебивает ESC-19). Whitepaper §11 «тейкдаун → воид → донору» не реализован (WP-3) — его отсутствие и создаёт вектор | **открыто** (нужно продукт-решение) | Фикс = §11 (тейкдаун активного спора → воид к донору). Задевает деньги (меняет дефолт) + вводит симметричный donor-collusion; в G3a бесплатно (резолвер уже умеет), в G3b нужен bounded ончейн-сигнал тейкдауна. Ждёт решения |
| CR-3 | LOW-MED | Бан кошелька (app-слой) блокирует `raiseDispute`/`vote` → подавляет голоса «не выполнил» | **принято** | На devnet избыточно (оператор=резолвер). Неустраним дешёвым патчем (бан обязан блокировать действия). Ответ — G3b commit-reveal + разрыв рычагов; см. CR-1 паттерн |
| CR-4 | HIGH | Текст ЗАДАНИЯ без ончейн-коммитмента: `task_id` — случайные байты, `Escrow`-struct без поля контента. Оператор мог СКРЫТЬ или ПОДМЕНИТЬ текст, который судит жюри, без крипто-проверки (инверсия относительно донатов с SHA-256) | закрыто | `task_id = SHA-256(nonce ‖ text)` (`taskTextCommitment`, moderation.ts) — сам ончейн-seed эскроу стал коммитментом, БЕЗ изменения программы/редеплоя. Сервер (`create`) требует совпадения (`verifyTextCommitment`→`ESCROW_TEXT_MISMATCH`, fail-closed без nonce); `nonce` хранится на задании для пересчёта третьей стороной |
| CR-5 | MED | Гейт создания задания — недетерминированный чёрный ящик (OpenAI + gpt-4o-mini judge за приватным ключом, вкл/выкл через env); `HARD_BLOCK` блокирует сам `fund` → выборочный отказ в игре под видом модерации | **принято** | Неустранимо патчем (модерация нелегальщины = централизованный вердикт). Митигация — мультиподпись/таймлок на операторские действия + пруф-якорь, G3b |
| CR-D1/D2 | — | Ончейн bounded resolver: выбирает сторону напрямую (D1, объявлено ADR 0017, G3b убирает); `mark_disputed` без ончейн-пруфа спора + post-deadline flip-race vs `resolve_timeout` (D2, сверх раскрытий ADR) | **известно/принято** | Ограничено 2 легитимными адресами (не кража/редирект). Убирается в G3b (ончейн commit-reveal) |
...
| ESC-19 | **HIGH** (продукт) | show-before-accept БЫЛ только офчейн-гейтом: контракт не видел текст (он офчейн, §4.6), и стример МИНУЯ UI мог `mark_done` из `Pending` → окно спора без диспута → `resolve_timeout` `ToStreamer` → `claim`, ни разу не показав текст (никто не узнал → не оспорил → невидимая выплата) | **закрыто** | **Контракт** (`lib.rs`, передеплой GPP2… на devnet): добавлена ончейн-инструкция `accept` (Pending→Accepted, signer=streamer); `mark_done` ТЕПЕРЬ требует `Accepted` → пути к деньгам стримеру без ончейн-`accept` нет. `cancel` расширен на Accepted-в-грейсе (паритет с офчейн). **Офчейн-раскрытие**: индексер (`settleDue → revealFromChain`, серверный хук `escrowState` → `readEscrowState`) видит ончейн-accept по PDA и раскрывает текст (`textState=SHOWN`) НЕЗАВИСИМО от UI; `machine.accept` тоже раскрывает. Итог: деньги ⟹ был accept ⟹ текст показан комьюнити ДО окна спора; чужим кошельком accept не пройдёт (signer=streamer). Проверено `escrow-smoke.ts` (mark_done без accept ревертит; accept→mark_done→claim 97/3) + vitest (accept раскрывает; settleDue раскрывает по escrowState) |
...
сейчас индексер декодирует инструкции, M3) — вернуть перед mainnet. **ESC-19 закрыт** (ончейн-`accept`-гейт
+ раскрытие текста индексером; передеплой на devnet). Зависимость M3: event-индексер обязан
