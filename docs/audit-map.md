...
> Источник механики — `docs/yellow-paper.md` (техспека по коду); риски — `docs/legal-and-risk.md`.
...
| §4.4 | **Репутация детерминирована/перевычислима** | `lib/reputation.ts` — чистые `pointsForAmount` (целочисленно, R1), `computePoints`, `resolveTier`; одинаковый журнал → одинаковая цифра. **С M0/M1 (ADR 0021) инвариант дополнительно держит кость №2**: `canister/core/src/{reputation,donation,indexer}.rs` — канистра ICP независимо пересобирает журнал ИЗ ЦЕПОЧКИ и отдаёт канон чтения (режим icp); паритет Rust↔TS — golden-тесты (`testdata/golden/`), сверка трёх источников — `verify-export --canister`; известные дельты dev-стенда — yellow-paper §18.5-8a |
| §4.5 | **Репутация только растёт; оператор её не редактирует** | `lib/reputation.ts → computePoints` (`Math.max(0, …)`); единственная отрицательная дельта — протокольный `DISPUTE_LOST` (проигранный ложный спор). Операторского ручного списания нет (`ADMIN_VOID` убран, CR-1) — наказание нарушителя — блок кошелька/канала, не правка числа |
...
| §8.9 спеки голосования | **Правила споров не меняются под идущий спор; площадка их не трогает** (новый замок M1, ADR 0021) | `canister/core/src/governance.rs` — параметры споров канала в канистре: запись ТОЛЬКО ed25519-подписью владельца (владелец = плательщик активации ИЗ ЖУРНАЛА цепочки, сервер не участвует), версия-нонс против replay, таймлок ≥ цикла спора; канон-сообщение запинено парными тестами (`src/lib/chain/dispute-params.test.ts` ↔ `governance.rs`) |
...
- **Аттестация payout (H1)** — `lib/chain/attestation.ts`: payout канала валиден только с ed25519-подписью
  владельца над каноническим сообщением. Проверяют: клиент донора (`chain-provider → assertPayoutAttested`
  ДО сборки донат-tx и эскроу-fund), сервер при создании/дозакреплении (`mock-provider → createChannel/
  attestPayout`) и при зачёте (`ingest`, chain-режим fail-closed). Сервер больше не источник истины по
  адресу выплат (ADR 0019).

### Прозрачность и перевычислимость (ADR 0019)

- **Публичный экспорт** — `GET /api/v1/export/channel/[handle]` (канал + аттестация + версии конфига +
  журнал + лидерборд) и `GET /api/v1/export/anchor` (полный журнал, конфиги, ПЕР-ЗАПИСНЫЕ хэши
  ОПЕРАТОРСКОГО лога: инцидент-лог + действия оператора — контент приватен и наружу не уходит, §4.6).
- **Независимый пересчёт** — `scripts/verify-export.ts`: репутация = свёртка журнала общим движком,
  подпись payout, дайджесты якоря; `--chain` сверяет якорь и донаты с цепочкой. §4.4 — кнопка, не декларация.
- **Пруф-якорь** — `server/anchor.ts` (+ тик в `indexer-service`): дайджесты журнала/конфигов/
  операторского лога (решения канальных модераторов стримера сюда не входят) периодически публикуются memo-tx (env `ANCHOR_SIGNER_KEYPAIR`, платит только свой газ —
  никакого ключа над чужими средствами, §4.1). Тихо переписать прошлое → расходится с ончейн-отпечатком.
...
| H1 | payout диктуется сервером | закрыто — ed25519-аттестация владельца (без нового контракта): клиент донора и ingest проверяют подпись; канал без подписи → донаты приостановлены (fail-closed). Остаточное доверие: привязка handle→owner — платформенная (задокументировано в `trust-layers.md`) | 0019 |
...
исходов §6, три замка §7, репутация §8, некастодиальность). Судьба нереализованных механик спеки:
**депозит спора ОТМЕНЁН владельцем** (2026-07-05, yellow-paper §18.5-8c — не возвращать); D_max и окна —
governance-параметры канистры (M1); commit-reveal ≥$50 — мейннет-план (§18.5-8b); рейт-лимиты — план.
**Реализовано 2026-07-02:** пороги репутации §10 как рычаги стримера — `minReputationToTask` (право прислать
задание) и `minReputationToDispute` (право поднять спор, заменил hardcode `DISPUTE_MIN_REP=1`), оба в
`ChannelConfig`/`ConfigPatch`, enforcement в `handlers.create`/`raiseDispute`, настройка в `/studio/settings`.
Ставит денежный барьер (репутация = задонатенные деньги) перед флудом заданий (MOD-4) и спамом споров (MOD-8).
Находки:
...
| WP-3 | — | Тейкдаун нелегальщины НЕ делает «спор воид + донат донору + депозит назад» (обещание v1.1 §12) — только снятие текста + инцидент + бан кошелька/канала | **задокументированное ограничение** | Гарантированный возврат оператором требует admin-key над эскроу → запрещён (§14 спеки, §4.1). Деньги идут по правилам эскроу; спека игры §12 исправлена под реальность |
...
|----|----------|---------|--------|---------|
| CR-1 | MED→HIGH | `ADMIN_VOID` (репутация=вес голоса) до снэпшота спора зануляет вес присяжного + голодит кворум → to_streamer. Рычаг неограничен (произвольная цель, нет привязки к нелегальщине, один ключ, лог самозаверенный) | закрыто (усилено) | **`ADMIN_VOID` убран полностью**: у оператора нет способа записать отрицательную дельту в журнал вообще → вес голоса всегда честный, §4.4 держится тривиально (никто не редактирует число). Наказание нарушителя — БЛОК (`BAN_WALLET_FULL` глобально / `CHANNEL_BLOCK` локально): вся ценность репутации обнуляется (не голосует/не спорит/не донатит), число остаётся честным. `DISPUTE_LOST` (протокол) — по-прежнему единственное списание, в весе учитывается. Промежуточный `computeVoteWeightAsOf` удалён как лишний |
| CR-2 | MED→HIGH | Тейкдаун текста в окне голосования ослепляет жюри → ниже кворума → to_streamer (`isContentBlocked`→`redactTask`, перебивает ESC-19). Whitepaper §11 «тейкдаун → воид → донору» не реализован (WP-3) | **принято (won't-fix на денежном слое)** — решение 2026-07-02 | Возврат донору просто перевесил бы подкрутку на другую сторону (оператор+донор) — чисто не решается ни одним дефолтом. Решено: НЕ вводить операторский денежный рычаг вообще (строгая некастодиальность важнее). Защита — на входе: сильная модерация задания ДО фандинга (`classifyTaskText`→HARD_BLOCK + CSAM-backstop, precheck перед fund). Раз-в-миллион просочившийся кейс — резолвится обычным жюри/таймаутом, спецмеханики нет. Отсутствие авто-возврата — намеренный ДЕТЕРРЕНТ: тролль рискует деньгами, если постит нелегальщину. Кода менять не нужно (текущее поведение = текст-онли тейкдаун, деньги по правилам эскроу). Зависимость: надёжность входной модерации (CR-5) |
| CR-3 | LOW-MED | Бан кошелька (app-слой) блокирует `raiseDispute`/`vote` → подавляет голоса «не выполнил» | **снято M2 для chain-задач** | С M2 голос/спор chain-задачи — подпись кошелька прямо в канистру (`/dispute/open`, `/dispute/vote`), сервер в пути не участвует → бан площадки голос физически не глушит (канистра проверяет только вес/стороны). Для оффчейн-задач (mock/api-эпоха) рычаг остаётся — принято |
...
| CR-D1/D2 | — | Ончейн bounded resolver: выбирает сторону напрямую (D1, объявлено ADR 0017); `mark_disputed` без ончейн-пруфа спора + post-deadline flip-race vs `resolve_timeout` (D2) | **закрыто M2** | RESOLVER = тресхолд-адрес канистры (ключа целиком не существует), сторона — из взвешенного голосования, ручной резолвер удалён; гонка с `resolve_timeout` закрыта клампом окна голосования к ончейн-дедлайну −40 с (yellow-paper §7.3). Bounded-свойство сохранено: перенаправить деньги третьему нельзя |
...
сейчас индексер декодирует инструкции, M3) — вернуть перед mainnet. **Upgrade authority эскроу-программы** —
один кошелёк (`G1vJ…uz14`) может заменить код программы, т.е. все ончейн-гарантии эскроу условны на честность
этого ключа: на devnet осознанное удобство итераций, перед mainnet — мультисиг + таймлок (окно, за которое
пользователи успевают выйти через permissionless `resolve_timeout`/`claim`), в идеале сжечь после G3b-аудита. **ESC-19 закрыт** (ончейн-`accept`-гейт
